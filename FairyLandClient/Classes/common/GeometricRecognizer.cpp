...
namespace DollarRecognizer
{
	...
	void GeometricRecognizer::loadTemplates()
	{
		...
		//addTemplate("CheckMark", samples.getGestureCWCheckMark());
        //addTemplate("CheckMark", samples.getGestureCCWCheckMark());
		addTemplate("Circle", samples.getGestureCCWCircle());
		addTemplate("Circle", samples.getGestureCWCircle());
        addTemplate("CircleRestore", samples.getGestureCWCircleRestore());
		...
		addTemplate("ObliqueLine", samples.getGestureLineDownDiagonal());
		...
		//addTemplate("Rectangle", samples.getGestureCCWRectangle());
		//addTemplate("Rectangle", samples.getGestureCWRectangle());
        //addTemplate("RectAdjust", samples.getGestureTemp());
		...
		//addTemplate("Triangle", samples.getGestureCCWTriangle());
		//addTemplate("Triangle", samples.getGestureCWTriangle());
        //addTemplate("TriangleRestore",samples.getGestureCWTriangleRestore());
        addTemplate("V", samples.getGestureCWV());
        addTemplate("V", samples.getGestureCCWV());
        //addTemplate("LV", samples.getGestureCCWLV());
        //addTemplate("VL", samples.getGestureCWVL());
	...
	cocos2d::CCRect GeometricRecognizer::boundingBox(Path2D points)
	{
		...
		return cocos2d::CCRectMake(minX, minY, (maxX - minX), (maxY - minY));
	}

	cocos2d::CCPoint GeometricRecognizer::centroid(Path2D points)
	{
		double x = 0.0, y = 0.0;
		for (Path2DIterator i = points.begin(); i != points.end(); i++)
		{
	...
	double GeometricRecognizer::getDistance(cocos2d::CCPoint p1, cocos2d::CCPoint p2)
	{
		double dx = p2.x - p1.x;
		double dy = p2.y - p1.y;
	...
	RecognitionResult GeometricRecognizer::recognize(Path2D points)
	{
		...
		if (-1 == indexOfBestMatch)
		{
			...
			return RecognitionResult("lineRestore", 1);
		}
	...
	Path2D GeometricRecognizer::resample(Path2D points)
	{
	    ...
	    for(int i = 1; i < (int)points.size(); i++)
		{
			cocos2d::CCPoint currentPoint  = points[i];
			cocos2d::CCPoint previousPoint = points[i-1];
	...
	{
		cocos2d::CCPoint c     = centroid(points);
	...
	Path2D GeometricRecognizer::scaleToSquare(Path2D points)
	{
		...
		cocos2d::CCRect box = boundingBox(points);
		Path2D newPoints;
		for (Path2DIterator i = points.begin(); i != points.end(); i++)
		{
			cocos2d::CCPoint point = *i;
			...
			double scaledX = point.x * (this->squareSize / box.size.width);
			double scaledY = point.y * (this->squareSize / box.size.height);
			...
			newPoints.push_back(ccp(scaledX, scaledY));
		}
		return newPoints;
	}
